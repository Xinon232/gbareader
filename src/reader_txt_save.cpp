...
bool parse_v1(const unsigned char*p,TxtSaveFooter&f){uint32_t o,c;if(!v1(p)||p[95]!='\n'||std::memcmp(p+1,"[GBAR-SAVE:1;O=",15)||p[27]!=';'||p[28]!='S'||p[29]!='='||p[31]!=';'||p[32]!='T'||p[33]!='='||p[35]!=';'||p[36]!='B'||p[37]!='='||p[39]!=';'||p[40]!='C'||p[41]!='='||!readdec(p+V1_OFFSET_AT,10,o)||!readhex(p+V1_CHECKSUM_AT,c)||c!=fnv(p,96,V1_CHECKSUM_AT,8))return false; if(p[30]<'1'||p[30]>'4'||p[34]<'1'||p[34]>'4'||p[38]<'1'||p[38]>'4')return false;f={};f.display_layout=0;f.byte_offset=o;f.settings={uint8_t(p[30]-'0'),uint8_t(p[34]-'0'),uint8_t(p[38]-'0')};f.history.lazy=o>0;f.history.lazy_anchor=o;return true;}
bool parse_v2(const unsigned char*p,TxtSaveFooter&f){if(std::memcmp(p,V2_MAGIC,sizeof(V2_MAGIC)-1)||p[383]!='\n'||get32(p+V2_SIZE_AT)!=384||get32(p+V2_CHECKSUM_AT)!=fnv(p,384,V2_CHECKSUM_AT,4))return false;int n=p[V2_HISTORY_COUNT_AT];if(n>PAGE_HISTORY_MAX||p[V2_SPACING_AT]<1||p[V2_SPACING_AT]>4||p[V2_TOP_AT]<1||p[V2_TOP_AT]>4||p[V2_BOTTOM_AT]<1||p[V2_BOTTOM_AT]>4)return false;f={};f.display_layout=0;f.byte_offset=get32(p+V2_OFFSET_AT);f.settings={p[V2_SPACING_AT],p[V2_TOP_AT],p[V2_BOTTOM_AT]};f.history.count=n;uint32_t last=0;for(int i=0;i<n;++i){uint32_t x=get32(p+V2_HISTORY_AT+i*4);if(x>=f.byte_offset||(i&&x<=last))return false;f.history.offsets[i]=x;last=x;}f.history.lazy=!n&&f.byte_offset;f.history.lazy_anchor=f.byte_offset;return true;}
}

void restore_saved_history(const TxtSaveFooter& footer, PageHistory& history,
                           PageHistoryRebuild& rebuild)
{
    if(footer.display_layout == CURRENT_DISPLAY_LAYOUT) {
        history = footer.history;
        rebuild = footer.history_rebuild;
        if(history.lazy && rebuild.state == HistoryRebuildState::IDLE)
            begin_history_rebuild(footer.byte_offset, rebuild);
    } else {
        history = {};
        history.lazy = footer.byte_offset > 0;
        history.lazy_anchor = footer.byte_offset;
        begin_history_rebuild(footer.byte_offset, rebuild);
    }
}
